...
const ChatItem = ({ chat, isSelected, onClick }: ChatItemProps) => {
    ...
    return (
        <div className={`${s.chatItem} ${isSelected ? s.selected : ''}`} onClick={onClick}>
            <div className={s.avatar}>
                {recipientAvatar ? (
                    <img src={recipientAvatar} alt={recipientName} />
                ) : (
                    <span className={s.avatarPlaceholder}>{getInitials(recipientName)}</span>
                )}
            </div>
            ...
            <div className={s.content}>
                <div className={s.header}>
                    ...
                    {chat.lastMessage?.createdAt && (
                        <span className={s.time}>{formatTime(chat.lastMessage.createdAt)}</span>
                    )}
                </div>
